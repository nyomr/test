  ...
  try {
    ...
    async function loadMoreJobsOnce() {
      ...
      try {
        ...
        await page.waitForSelector(loadMoreButtonSelector, { visible: true, timeout: 20000 }); // Perpanjang batas waktu menjadi 10 detik
        ...
        await page.waitForFunction(
          () => new Promise(resolve => setTimeout(resolve, 10000)) // Perpanjang waktu tunggu menjadi 5 detik
        );
    ...
    // Tekan tombol "Load more jobs" sekali untuk memuat data tambahan
    const loadMoreResult = await loadMoreJobsOnce();

    if (!loadMoreResult) {
      console.warn('Load more jobs button was not clicked.');
    }
